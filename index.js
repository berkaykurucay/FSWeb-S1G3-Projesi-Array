...
function kopyala(orijinalTatlar) {
  const sonuc = orijinalTatlar.slice();
  return sonuc;
}
/* Görev 2:
...
console.log("indekstekiçeşit", indekstekiCesitiGetir(orijinalTatlar, 2));
/* Görev 6:
...
function ortalamaKelimeSayisi(dizi) {
  ...
  const ortalama = toplamKelimeSayisi / dizi.length;
  return ortalama;
}
...
function rastgeleTatlar(orijinalTatlar, yeniTatlar, mevsimlikTatlar, bolgeselTatlar)
  ...
  while (rastgeleTatlar.length < 25) {
    ...
    const rastgeleTatlar = tumTatlar[rastgeleIndex];
